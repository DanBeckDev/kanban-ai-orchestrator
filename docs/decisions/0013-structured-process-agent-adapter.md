...
- Codex CLI, Claude Code, and the provider-neutral structured bridge pass the same non-interactive conformance suite. The suite verifies explicit capability discovery, non-resumable sessions, rejected feedback/resume/interruption, sequential safe events, and a terminal review-ready completion.
