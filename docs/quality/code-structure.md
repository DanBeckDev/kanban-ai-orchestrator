...
`npm run structure:check` checks the current working-tree change; CI checks every pull-request change against its base branch. The only temporary exceptions are listed in `code-structure-exceptions.json`. Each needs an owner work item, expiry date, and a fixed maximum meaningful-line count. Once an exception is present on the base branch, CI and the local hook reject a changed owner, a later expiry, a larger ceiling, or a new exception. An exception may never be used to grow its file; the file must stay at or below the recorded ceiling while it is being split. No new exception is permitted without an accepted ADR and product-owner approval; expired entries fail the gate.
