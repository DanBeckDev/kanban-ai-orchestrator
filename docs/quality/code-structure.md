...
For changed source, the quality-review receipt must inventory each source path, its exact meaningful-line count, the cohesive responsibility assessed, and the decision. The receipt gate calculates the count again and rejects a missing or stale entry. This makes vertical-density review evidence visible without turning a line count below 400 into a presumption of cohesion.

`npm run structure:check` checks the current working-tree change, including untracked source files. `npm run structure:verify` scans the entire repository, including untracked source files; it runs in both the local quality gate and every full-quality verification. The exception ledger is intentionally retained as an auditable empty record, but any entry fails the gate. An agent must split the code instead of recording an exception.
